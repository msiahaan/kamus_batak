...
# list of drivers will be build on the fly
...
def sqlhtml_validators(field_type, length):
    ...
    if field_type == 'string':
        return [validators.IS_LENGTH(length)]
    elif field_type == 'text':
        return [validators.IS_LENGTH(2 ** 16)]
    ...
    elif field_type[:10] == 'reference ':
        return [validators.IS_INT_IN_RANGE(0, 1e100)]
    else:
        return []
...
class SQLDB(SQLStorage):
    ...
    def __init__(self, uri='sqlite://dummy.db', pool_size=0, pools=0):
        self._uri = str(uri) # NOTE: assumming it is in utf8!!!
...
class SQLALL(object):
    """
    Helper class providing a comma seperated string having all the field names
    (prefix by table name and '.')
...
class SQLTable(dict):
    ...
    def __init__(
        ...
        *fields
        ...
        """
        initializes the table and performs checjing on the provided fields.

        Each table will have automaticly an 'id'.
        ...
        for field in fields:
            if isinstance(field, SQLField):
                ...
                raise SyntaxError, \
                    'define_table argument is not a SQLField: %s' % field
        fields = new_fields
        self._db = db
    ...
    def _create(self, migrate=True):
        ...
        for k in self.fields:
            ...
            if isinstance(field.type,SQLCustomType):
                ...
                raise SyntaxError, 'SQLField: unkown field type: %s for %s' % \
        ...
        # backend specific etxensions to fields
...
class SQLCustomType:
    """
    allows for defining custom SQL types
    ...
    :param requires: what validators to use ( default = None, will use the
...
class SQLField(SQLXorable):
    ...
    example::
        ...
        a = SQLField(name, 'string', length=32, default=None, required=False, 
    ...
    def __init__(
        ...
        if hasattr(SQLTable,fieldname) or fieldname[0] == '_':
...
class SQLQuery(object):
    ...
    :raises SyntaxError: when the query cannot be recognised
    """
...
class SQLRows(object):
	...
	"""
        returns the data as a list or dictionary. Hooks are not invoked.
        ...
        def dictit(d):
            """performing the actual list providing"""
