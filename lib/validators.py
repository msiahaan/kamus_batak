...
class Validator(object):
    ...
    Builtin validators have constructors that take the optional argument error message
    which allows you to change the default error message.
...
class IS_EMAIL(Validator):
    ...
    regex = re.compile('^([0-9a-zA-Z]([-.\w]*[0-9a-zA-Z])*@([0-9a-zA-Z][-\w]*[0-9a-zA-Z]\.)+[a-zA-Z]{2,9})$')
...
def escape_unicode(string):
    ...
    :returns: the US-ASCII escaped form of the inputed string
    :rtype: string
...
def unicode_to_ascii_authority(authority):
    ...
    :rtype: string
    :raises Exception: if the function is not able to convert the inputed authority
...
class IS_GENERIC_URL(Validator):
    ...
    def __call__(self, value):
        ...
        :returns: a tuple, where tuple[0] is the inputed value (possible prepended with prepend_scheme),
...
class IS_URL(Validator):
    ...
    def __call__(self, value):
        ...
        :returns: a (string, string) tuple, where tuple[0] is the modified input value and tuple[1] is either
        ...
        if type(self.special) == int:
            ...
            if self.special > 0:
                if not all_special.count(True) >= self.special:
                    failures.append("Must include " + str(self.special) + " of the following : " + self.specials)
        if self.invalid:
            all_invalid = [ch in value for ch in self.invalid]
            if all_invalid.count(True) > 0:
                failures.append("Cannot contain any of the following: " + self.invalid)
        if type(self.upper) == int:
            all_upper = re.findall("[A-Z]", value)
            if self.upper > 0:
                if not len(all_upper) >= self.upper:
                    failures.append("Must include " + str(self.upper) + " upper case")
            else:
                if len(all_upper) > 0:
                    failures.append("Cannot include upper case letters")
        if type(self.lower) == int:
            all_lower = re.findall("[a-z]", value)
            if self.lower > 0:
                if not len(all_lower) >= self.lower:
                    failures.append("Must include " + str(self.lower) + " lower case")
            else:
                if len(all_lower) > 0:
                    failures.append("Cannot include lower case letters")
        if type(self.number) == int:
            all_number = re.findall("[0-9]", value)
            if self.number > 0:
                if not len(all_number) >= self.number:
                    failures.append("Must include " + str(self.number) + " numbers")
            else:
                if len(all_number) > 0:
                    failures.append("Cannot include any numbers")
        if len(failures) == 0:
            return (value, None)
...
class IS_UPLOAD_FILENAME(Validator):
    ...
    def __init__(self, filename=None, extension=None, lastdot=True, case=1, error_message='invalid filename or extension!'):
...
class IS_IPV4(Validator):
    ...
    be set to force adresses from certain range.
    ...
    def __init__(self, minip='0.0.0.0', maxip='255.255.255.255', error_message='invalid IPv4 address!'):
